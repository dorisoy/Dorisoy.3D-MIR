...
from invesalius.gui.custom_windows import WindowStyleManager
...
class InVesalius(wx.App):
    ...
    def OnInit(self):
        ...
        # Initialize WindowStyleManager right after wx.App is created
        WindowStyleManager._initialize()

        self.SetAppName("Sinol.3D-MIR")
...
class Inv3SplashScreen(SplashScreen):
    ...
    def __init__(self):
        ...
        lang = LANG
        # 根据用户语言设置 locale，使 wxPython 标准按钮显示正确的语言
        if lang and lang.startswith("zh"):
            # 中文
            self.locale = wx.Locale(wx.LANGUAGE_CHINESE_SIMPLIFIED if "_CN" in lang else wx.LANGUAGE_CHINESE_TRADITIONAL)
        else:
            # 默认英文
    ...
    def Startup(self):
        ...
        # p: Thread = Thread(target=utils.UpdateCheck, args=())
        # p.start()
...
def parse_command_line():
    ...
    parser.add_argument("project_file", nargs="?", default="", help="Sinol.3D-MIR project file")
...
def init():
    ...
    if hasattr(sys, "frozen") and sys.platform == "win32":
        ...
        key = r"Sinol.3D-MIR.1\InstallationDir"
